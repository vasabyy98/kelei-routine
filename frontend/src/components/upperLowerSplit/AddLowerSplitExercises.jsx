...
import { setLowerSplitExercises, setUnit } from "../../features/plans/planDraftSlice";
...
function AddExercises() {
  ...
  const [lowerSplitExercises, setLowerExercises] = useState(draft.lowerSplitExercises);
  const [massUnit, setMassUnit] = useState("kg");

  const lowerSplitSection = useRef();

  const nextLink = useRef();

  const addExercise = (exerciseInfo, unitInfo) => {
    setLowerExercises((prev) => [...prev, exerciseInfo]);
    setMassUnit(unitInfo);
  };

  const removeExercise = (e) => {
    let val = e.currentTarget.getAttribute("name").toLowerCase();
    setLowerExercises(lowerSplitExercises.filter((exercise) => exercise.exercise !== val));
  };
  ...
  useEffect(() => {
    ...
    if (lowerSplitExercises.length > 0) {
      nextLink.current.classList.remove(btnStyles.disabledBtn);
    } else {
      nextLink.current.classList.add(btnStyles.disabledBtn);
    }

    dispatch(setLowerSplitExercises(lowerSplitExercises));
  }, [lowerSplitExercises]);
  return (
    <>
      <section ref={lowerSplitSection} className={`${layout.content__wrapper__bg} `}>
        <div className={layout.twoRow__grid__layout}>
          <div className={layout.flex__layout}>
            <Header line1={"add your"} line2={"lower split"} line3={"exercises"} />
            <div className={layout.added__exercises}>
              <ExerciseForm
                addExercise={addExercise}
                show={show}
                onClose={() => setShow(false)}
                section={lowerSplitSection}
              />
              <ExerciseList
                fullbodyExercises={lowerSplitExercises}
                massUnit={massUnit}
